...
class MegaWalker extends \Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
	{
		...
		if ($has_feature_image && $this->mege_menu_id !== 0) {
			...
			$field_menu_items_image = get_field('field_menu_items_image', $item);
			if ($field_menu_items_image && $this->mege_menu_id !== 0) {
				$item_output .= '<img alt="' . esc_attr($item->title) . '" src="' . $field_menu_items_image . '"/>';
			}
		}
